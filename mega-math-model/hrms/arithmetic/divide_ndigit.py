import random
from hrms.base import HRMBase

class DivideNDigitHRM(HRMBase):
    def __init__(self, name="Divide_Ndigit", d_model=128, vocab_size=128):
        super().__init__(name=name, vocab_size=vocab_size, d_model=d_model)

    def forward(self, tokens):
        try:
            raw = "".join(chr(int(t)) for t in tokens[0] if int(t) < 128)
        except:
            raw = str(tokens)

        raw = raw.replace("÷","/")

        try:
            a_s, b_s = raw.split("/")
            a = int(a_s)
            b = int(b_s)
            out = "Undefined" if b == 0 else f"{a//b} rem {a%b}"
        except:
            out = raw

        return [[c for c in out]], {"conf": 0.88}

def make_dataset(n=30000, digits=3):
    ds = []
    lo = 10**(digits-1)
    hi = 10**digits - 1
    for _ in range(n):
        a = random.randint(lo,hi)
        b = random.randint(1,hi)
        ds.append((f"{a}/{b}", f"{a//b} rem {a%b}"))
    return ds
